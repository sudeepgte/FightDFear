...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin:0; padding:0; }
    ...
    .card { background:#fff; border-radius:16px; box-shadow:0 2px 12px rgba(0,0,0,0.05); overflow:hidden; }
    .card-header { padding:16px 22px; border-bottom:1px solid #f1f0f7; display:flex; align-items:center; justify-content:space-between; }
    .card-title { font-size:1rem; font-weight:800; color:#1e1b4b; display:flex; align-items:center; gap:8px; }
    .search-wrap { position:relative; }
    .search-wrap i { position:absolute; left:12px; top:50%; transform:translateY(-50%); color:#aaa; }
    .search-input { border:1.5px solid #e5e7eb; border-radius:10px; padding:8px 14px 8px 36px; font-family:'Outfit',sans-serif; font-size:0.85rem; outline:none; width:200px; }
    .search-input:focus { border-color:#6d28d9; }
    table { width:100%; border-collapse:collapse; }
    th { padding:11px 16px; text-align:left; font-size:0.75rem; font-weight:700; color:#aaa; text-transform:uppercase; letter-spacing:0.5px; background:#faf9ff; }
    td { padding:13px 16px; border-top:1px solid #f5f0fb; font-size:0.875rem; vertical-align:middle; }
    tr:hover td { background:#faf5ff; }
    .user-cell { display:flex; align-items:center; gap:10px; }
    .u-avatar { width:34px; height:34px; border-radius:50%; background:linear-gradient(135deg,#6d28d9,#a855f7); display:flex; align-items:center; justify-content:center; color:#fff; font-weight:700; font-size:0.85rem; flex-shrink:0; }
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title"><i class="bi bi-people-fill"></i> All Registrations
                    <span style="background:#ede9fe;color:#6d28d9;border-radius:12px;padding:2px 10px;font-size:0.75rem;">${fn:length(allRegistrations)}</span>
                </div>
                            ...
                            <tbody>
                                <c:forEach var="reg" items="${allRegistrations}">
                                    <tr data-search="${fn:toLowerCase(reg.user.fullName)} ${fn:toLowerCase(reg.event.name)}">
                                        ...
                                        <td>
                                            <div style="font-weight:700;font-size:0.87rem;color:#1e1b4b;">${reg.event.name}</div>
                                            <div style="font-size:0.73rem;color:#aaa;">${reg.event.city}</div>
                                        </td>
                                        <td style="font-size:0.83rem;color:#555;">${reg.registeredAt}</td>
                                        <td><code style="background:#f5f0ff;color:#6d28d9;padding:3px 8px;border-radius:6px;font-size:0.8rem;">${reg.ticketCode}</code></td>
