...
class FitnessTrainerProfileCompletionScreen extends StatefulWidget {
  const FitnessTrainerProfileCompletionScreen({super.key, this.onFinished});
  ...
  @override
  State<FitnessTrainerProfileCompletionScreen> createState() =>
      _FitnessTrainerProfileCompletionScreenState();
}

class _FitnessTrainerProfileCompletionScreenState
    extends State<FitnessTrainerProfileCompletionScreen> {
  ...
  final _rate = TextEditingController(text: '300');
  ...
  FitnessTrainerAuthService get _svc =>
      FitnessTrainerAuthService(context.read<AuthState>().api);
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _years.text = (p['experience'] ?? p['experienceYears'] ?? p['yearsExperience'])?.toString() ?? '';
    _credential.text = p['credentialNumber']?.toString() ?? '';
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', FitnessCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(FitnessCatalog.splitCsv(p['specializations'] ?? p['categoriesOffered']));
    _audience
      ..clear()
      ..addAll(FitnessCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(FitnessCatalog.splitCsv(p['facilities']));
    _days
      ..clear()
      ..addAll(FitnessCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(FitnessCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = FitnessCatalog.parseTime(p['openTime']?.toString());
    _closeTime = FitnessCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = FitnessCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = FitnessCatalog.parseTime(p['breakEnd']?.toString());
    _bio.text = p['bio']?.toString() ?? '';
    _upi.text = p['upiId']?.toString() ?? '';
    _bank.text = p['bankDetails']?.toString() ?? '';
    if (p['typicalPrice'] != null || p['sessionFees'] != null) {
      _rate.text = '${p['typicalPrice'] ?? p['sessionFees']}';
    }
    ...
    _sessionMode = p['sessionMode']?.toString() ?? p['serviceType']?.toString();
    if ((_sessionMode ?? '').isEmpty) _sessionMode = FitnessCatalog.sessionModes.first;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'experience': int.tryParse(_years.text.trim()),
      ...
      'latitude': _lat,
      'longitude': _lng,
      'specializations': _categories.toList(),
      ...
      'openTime': _openTime == null ? null : FitnessCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : FitnessCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : FitnessCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : FitnessCatalog.formatTime(_breakEnd!),
      'blockedDates': _blockedDates.toList(),
      'bio': _bio.text.trim(),
      'typicalPrice': double.tryParse(_rate.text.trim()) ?? 0,
      'sessionFees': double.tryParse(_rate.text.trim()) ?? 0,
      'sessionMode': _sessionMode,
      'serviceType': _sessionMode,
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if ((_designation ?? '').isEmpty) return '1.2 Designation is required';
    if (_phone.text.trim().length != 10) return '1.5 Official phone must be 10 digits';
    if (_years.text.trim().isEmpty) return '1.7 Years of experience is required';
    if (_credential.text.trim().isEmpty) return '1.8 ACE / NASM / Yoga Alliance / cert number is required';
    ...
    if (_categories.isEmpty) return '3.1 Select at least one specialization';
    ...
    if ((_sessionMode ?? '').isEmpty) return '8. Session mode is required';
    if (double.tryParse(_rate.text.trim()) == null) return '8. Typical session fee is required (0 = free)';
  ...
  Future<void> _pickClock(void Function(TimeOfDay t) apply, TimeOfDay? current) async {
    final picked = await showTimePicker(context: context, initialTime: current ?? const TimeOfDay(hour: 6, minute: 0));
  ...
  Future<void> _useCurrentLocation() async {
    ...
    if (perm == LocationPermission.denied || perm == LocationPermission.deniedForever) {
      if (mounted) setState(() => _error = 'Location permission is needed to pin your studio');
      return;
    }
  ...
  Future<void> _uploadFile({required String kind}) async {
    ...
    try {
      final res = await _svc.uploadPhotos(
        ...
        certificatePath: kind == 'cert' ? path : null,
      );
      if (!mounted) return;
      if (res['success'] == true) {
        await _load();
  ...
  Widget build(BuildContext context) {
    ...
    final missing = FitnessCatalog.splitCsv(_profile['missingItems']);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: FitnessTrainerProfileCompletionScreen.navy,
        title: const Text('Complete Trainer Profile', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
      body: _loading
              ...
              children: [
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: FitnessTrainerProfileCompletionScreen.primary,
                ...
                _section('1', 'Trainer identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _dropdown(
                    number: '1.2',
                    label: 'Designation',
                    value: _designation,
                    options: FitnessCatalog.designations,
                    required: true,
                  ...
                  _field(_years, '1.7', 'Years of experience', required: true, keyboardType: TextInputType.number, maxLength: 2, inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
                  _field(_credential, '1.8', 'ACE / NASM / Yoga Alliance / cert number', required: true, hint: 'Type credential number'),
                ]),
                _section('2', 'Location', [
                  _field(_address, '2.1', 'Studio / home address', required: true),
                  _field(_city, '2.3', 'City', required: true),
                  _dropdown(number: '2.4', label: 'State', value: _state, options: FitnessCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                  ...
                  const SizedBox(height: 8),
                  const Text('2.7 Pin studio on map', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  ...
                  SizedBox(
                    ...
                    child: ClipRRect(
                      ...
                      child: GoogleMap(
                        ...
                        markers: {
                          if (_lat != null && _lng != null)
                            Marker(markerId: const MarkerId('studio'), position: LatLng(_lat!, _lng!)),
                        },
                        onTap: (pos) => setState(() {
                          _lat = pos.latitude;
                          _lng = pos.longitude;
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(FitnessCatalog.audiences, _audience),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('4.2 Home / doorstep sessions'),
                    value: _doorService,
                    onChanged: (v) => setState(() => _doorService = v),
                  ),
                ]),
                _section('5', 'Facilities', [
                  const Text('5.1 Studio amenities', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  _chips(FitnessCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: FitnessCatalog.days.map((d) {
                ...
                _section('8', 'Typical session', [
                  _dropdown(
                    number: '8.1',
                    label: 'Session mode',
                    value: _sessionMode,
                    options: FitnessCatalog.sessionModes,
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_duration',
                    options: FitnessCatalog.durations.map((e) => '$e').toList(),
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_buffer',
                    options: FitnessCatalog.buffers.map((e) => '$e').toList(),
                    onChanged: (v) => setState(() => _buffer = int.tryParse(v ?? '10') ?? 10),
                  ),
                  _field(_rate, '8.4', 'Typical session fee (₹, 0 = free)', required: true, keyboardType: TextInputType.number),
                ]),
                ...
                _section('10', 'Documents (optional)', [
                  ...
                  const SizedBox(height: 10),
                  _uploadTile('10.1 Profile photo', _profile['profileImageUrl']?.toString() ?? _profile['profilePhotoPath']?.toString(), _uploadingPhoto, () => _uploadFile(kind: 'photo')),
                  _uploadTile('10.2 Certificate scan', _profile['certificationsPath']?.toString(), _uploadingCert, () => _uploadFile(kind: 'cert')),
                ]),
                _section('11', 'Studio photos (optional)', [
                  const Text('Photos help clients choose you.', style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                  const SizedBox(height: 10),
                  _uploadTile(
                    '11.1 Gallery photo',
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : FitnessCatalog.formatTime(value)),
