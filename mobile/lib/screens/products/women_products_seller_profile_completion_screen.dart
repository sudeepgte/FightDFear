...
class WomenProductsSellerProfileCompletionScreen extends StatefulWidget {
  const WomenProductsSellerProfileCompletionScreen({super.key, this.onFinished});
  ...
  @override
  State<WomenProductsSellerProfileCompletionScreen> createState() =>
      _WomenProductsSellerProfileCompletionScreenState();
}

class _WomenProductsSellerProfileCompletionScreenState
    extends State<WomenProductsSellerProfileCompletionScreen> {
  ...
  final _rate = TextEditingController(text: '499');
  ...
  void dispose() {
    for (final c in [
      ...
      _pincode, _mapLocation, _bio, _upi, _bank, _gstin, _rate, _shop,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _shop.text = p['businessName']?.toString() ?? '';
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', SellerCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(SellerCatalog.splitCsv(p['categoriesOffered']));
    _primaryCategory = p['primaryCategory']?.toString();
    if ((_primaryCategory ?? '').isEmpty) _primaryCategory = _categories.isEmpty ? null : _categories.first;
    _brandType = p['brandType']?.toString();
    if ((_brandType ?? '').isEmpty) _brandType = null;
    _audience
      ..clear()
      ..addAll(SellerCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(SellerCatalog.splitCsv(p['facilities']));
    _days
      ..clear()
      ..addAll(SellerCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(SellerCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = SellerCatalog.parseTime(p['openTime']?.toString());
    _closeTime = SellerCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = SellerCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = SellerCatalog.parseTime(p['breakEnd']?.toString());
    _bio.text = p['bio']?.toString() ?? '';
    _upi.text = p['upiId']?.toString() ?? '';
    _bank.text = p['bankDetails']?.toString() ?? '';
    _gstin.text = p['gstin']?.toString() ?? '';
    if (p['typicalPrice'] != null) _rate.text = '${p['typicalPrice']}';
    _dispatch = p['dispatchHours'] is num ? (p['dispatchHours'] as num).toInt() : 24;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'primaryCategory': _primaryCategory ?? (_categories.isEmpty ? null : _categories.first),
      'brandType': _brandType,
      'audience': _audience.toList(),
      'doorService': _doorService,
      'facilities': _facilities.toList(),
      'openDays': _days.toList(),
      'openTime': _openTime == null ? null : SellerCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : SellerCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : SellerCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : SellerCatalog.formatTime(_breakEnd!),
      'blockedDates': _blockedDates.toList(),
      'bio': _bio.text.trim(),
      'typicalPrice': double.tryParse(_rate.text.trim()) ?? 0,
      'dispatchHours': _dispatch,
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if (_shop.text.trim().isEmpty) return '1. Shop name is required';
    ...
    if ((_primaryCategory ?? '').isEmpty) return '8. Primary category is required';
    if ((_brandType ?? '').isEmpty) return '4.1 Brand type is required';
    if (double.tryParse(_rate.text.trim()) == null) return '8. Typical price is required';
  ...
  Widget build(BuildContext context) {
    ...
    final missing = SellerCatalog.splitCsv(_profile['missingItems']);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: WomenProductsSellerProfileCompletionScreen.navy,
        title: const Text('Complete Shop Profile', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
      body: _loading
              ...
              children: [
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: WomenProductsSellerProfileCompletionScreen.primary,
                ...
                _section('1', 'Seller identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _field(_shop, '1.3', 'Shop name', required: true),
                  _dropdown(
                    number: '1.2',
                    label: 'Role',
                    value: _designation,
                    options: SellerCatalog.designations,
                    required: true,
                ...
                _section('2', 'Location', [
                  ...
                  _dropdown(number: '2.4', label: 'State', value: _state, options: SellerCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                  ...
                  const SizedBox(height: 8),
                  const Text('2.7 Pin shop on map', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                ...
                _section('3', 'Categories you sell', [
                  const Text('3.1 Categories you sell *', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: SellerCatalog.categories.map((c) {
                      ...
                      return FilterChip(
                        ...
                        onSelected: (v) => setState(() {
                          if (v) {
                            ...
                            _primaryCategory ??= c.code;
                          } else {
                            _categories.remove(c.code);
                            if (_primaryCategory == c.code) {
                              _primaryCategory = _categories.isEmpty ? null : _categories.first;
                            }
                          }
                        }),
                      );
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(SellerCatalog.audiences, _audience),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('4.2 Local pickup'),
                    value: _doorService,
                    onChanged: (v) => setState(() => _doorService = v),
                  ),
                ]),
                _section('5', 'Shop facilities', [
                  const Text('5.1 Shop amenities', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  _chips(SellerCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: SellerCatalog.days.map((d) {
                ...
                _section('7', 'About the shop', [
                  _field(_bio, '7.1', 'About', required: true, maxLines: 4),
                ]),
                _section('8', 'First listing defaults', [
                  _dropdown(
                    number: '8.1',
                    label: 'Primary category',
                    value: _primaryCategory,
                    options: SellerCatalog.categories.map((c) => c.code).toList(),
                    required: true,
                    onChanged: (v) => setState(() {
                      _primaryCategory = v;
                      if (v != null) _categories.add(v);
                    }),
                  ),
                  _dropdown(
                    number: '8.2',
                    label: 'Brand type',
                    value: _brandType,
                    options: SellerCatalog.brandTypes,
                    required: true,
                    onChanged: (v) => setState(() => _brandType = v),
                  ),
                  _dropdown(
                    number: '8.3',
                    label: 'Dispatch hours',
                    value: '$_dispatch',
                    options: SellerCatalog.dispatchHours.map((e) => '$e').toList(),
                    required: true,
                    onChanged: (v) => setState(() => _dispatch = int.tryParse(v ?? '24') ?? 24),
                  ),
                  _field(_rate, '8.4', 'Typical price (₹)', required: true, keyboardType: TextInputType.number),
                ]),
                _section('9', 'Payout', [
                  _field(_upi, '9.1', 'UPI ID', hint: 'name@upi'),
                  _field(_bank, '9.2', 'Bank details', hint: 'Account name, number, IFSC', maxLines: 2),
                  _field(_gstin, '9.3', 'GSTIN', hint: 'Optional'),
                  const Text(
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : SellerCatalog.formatTime(value)),
