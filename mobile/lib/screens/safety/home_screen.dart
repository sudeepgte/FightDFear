...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      if (!mounted) return;
      if (active['_status'] == 401) {
        setState(() {
          _banner = 'Session expired — please sign in again to use SOS';
      ...
      if (active['success'] == true && active['active'] == true) {
        ...
        _banner = active['error']?.toString() ?? 'Could not check SOS status';
      }
    } catch (e) {
    ...
    try {
      ...
      if (res['success'] == true) {
        ...
        setState(() {
          ...
          _triggering = false;
          _banner = res['alreadyActive'] == true
              ? (res['message']?.toString() ?? 'SOS already active')
        ...
        if (id != null) {
          await _refreshStatus();
          _startPolling();
          if (res['alreadyActive'] != true) {
            await _autoAlertChannels(res);
          }
        }
      } else if (res['_status'] == 401) {
        setState(() {
          _triggering = false;
          _banner = 'Session expired — sign in again to send SOS';
        });
      } else {
        setState(() {
          _triggering = false;
  ...
  /// Auto-dial trusted contact (or 100), then open WhatsApp with live location.
  /// SMS + email are already sent by the backend during /api/sos/trigger.
  Future<void> _autoAlertChannels(Map<String, dynamic> res) async {
    var phone = _autoCallPhone;
    if (phone == null || phone.isEmpty || phone == 'null') {
      phone = '100';
      _autoCallPhone = phone;
    }

    setState(() => _banner = 'Auto-calling $phone…');
    await _callEmergency();

    final shares = res['whatsappShares'];
    if (shares is! List || shares.isEmpty) {
      if (mounted) {
        setState(() {
          _banner =
              '${_banner ?? 'SOS active'} · SMS/email sent by server when configured';
        });
      }
      return;
    }

    // Open first WhatsApp chat with pre-filled SOS + location (user taps Send once).
    // Silent multi-send needs WhatsApp Business API — not available on-device.
    final first = Map<String, dynamic>.from(shares.first as Map);
    final waRaw = first['whatsappNumber']?.toString() ?? '';
    final digits = waRaw.replaceAll(RegExp(r'\D'), '');
    ...
    final text = first['message']?.toString() ??
        '🚨 EMERGENCY SOS\n📍 ${_mapsLink ?? ''}';
    final uri = Uri.parse(
      'https://wa.me/$withCountry?text=${Uri.encodeComponent(text)}',
    );

    if (!mounted) return;
    setState(() {
      _banner =
          'Opening WhatsApp for ${first['name'] ?? 'contact'} with your live location…';
    });
    ...
    // Queue remaining WhatsApp shares as quick actions via snackbar hint
    if (shares.length > 1 && mounted) {
      _showSnack(
        '${shares.length - 1} more WhatsApp contact(s) — open Trusted Contacts or share Maps again if needed',
      );
    }
  }

  Future<void> _callEmergency() async {
    final phone = (_autoCallPhone == null ||
            _autoCallPhone!.isEmpty ||
            _autoCallPhone == 'null')
        ? '100'
        : _autoCallPhone!;
    ...
    try {
      ...
      if (!ok && mounted) {
        _showSnack('Could not open dialer for $phone');
      }
    } catch (_) {
      if (mounted) _showSnack('Cannot open phone dialer');
    }
  }
                  ...
                  child: Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Add trusted contacts first so SOS can notify them. Tap to manage.',
              ...
              Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.contacts_outlined, color: Color(0xFF15803D)),
                        const SizedBox(width: 10),
                        Expanded(
                          child: Text(
                            '$contactCount trusted contact${contactCount == 1 ? '' : 's'} ready for SOS alerts',
