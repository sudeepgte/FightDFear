...
class CreatorProfileCompletionScreen extends StatefulWidget {
  const CreatorProfileCompletionScreen({super.key, this.onFinished});
  ...
  @override
  State<CreatorProfileCompletionScreen> createState() =>
      _CreatorProfileCompletionScreenState();
}

class _CreatorProfileCompletionScreenState
    extends State<CreatorProfileCompletionScreen> {
  ...
  CreatorAuthService get _svc =>
      CreatorAuthService(context.read<AuthState>().api);
  ...
  void dispose() {
    for (final c in [
      _name, _handle, _phone, _whatsapp, _years, _address, _city, _stateOther,
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _handle.text = p['handle']?.toString() ?? '';
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', CreatorCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(CreatorCatalog.splitCsv(p['expertise'] ?? p['categoriesOffered']));
    _audience
      ..clear()
      ..addAll(CreatorCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(CreatorCatalog.splitCsv(p['facilities']));
    _days
      ..clear()
      ..addAll(CreatorCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(CreatorCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = CreatorCatalog.parseTime(p['openTime']?.toString());
    _closeTime = CreatorCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = CreatorCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = CreatorCatalog.parseTime(p['breakEnd']?.toString());
    ...
    if ((_sessionMode ?? '').isEmpty) _sessionMode = CreatorCatalog.sessionModes.first;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'openTime': _openTime == null ? null : CreatorCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : CreatorCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : CreatorCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : CreatorCatalog.formatTime(_breakEnd!),
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if (_credential.text.trim().isEmpty) return '1.8 PAN / GST / channel ID is required';
  ...
  Widget build(BuildContext context) {
    ...
    final missing = CreatorCatalog.splitCsv(_profile['missingItems']);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: CreatorProfileCompletionScreen.navy,
        title: const Text('Complete Creator Profile', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
      body: _loading
              ...
              children: [
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: CreatorProfileCompletionScreen.primary,
                ...
                _section('1', 'Creator identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _field(_handle, '1.3', 'Handle'),
                  _dropdown(
                    number: '1.2',
                    label: 'Role',
                    value: _designation,
                    options: CreatorCatalog.designations,
                    required: true,
                  ...
                  _field(_credential, '1.8', 'PAN / GST / channel ID', required: true, hint: 'PAN, GSTIN, or channel / page ID'),
                ]),
                _section('2', 'Location', [
                  _field(_address, '2.1', 'Address', required: true),
                  _field(_city, '2.3', 'City', required: true),
                  _dropdown(number: '2.4', label: 'State', value: _state, options: CreatorCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(CreatorCatalog.audiences, _audience),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('4.2 In-person / vernacular shoots'),
                    value: _doorService,
                ...
                _section('5', 'Facilities', [
                  ...
                  const SizedBox(height: 8),
                  _chips(CreatorCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: CreatorCatalog.days.map((d) {
                ...
                _section('8', 'First offering', [
                  _dropdown(
                    ...
                    label: 'Content mode',
                    value: _sessionMode,
                    options: CreatorCatalog.sessionModes,
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_duration',
                    options: CreatorCatalog.durations.map((e) => '$e').toList(),
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_buffer',
                    options: CreatorCatalog.buffers.map((e) => '$e').toList(),
                    onChanged: (v) => setState(() => _buffer = int.tryParse(v ?? '10') ?? 10),
                  ),
                  _field(_rate, '8.4', 'Typical price (₹, 0 = free)', required: true, keyboardType: TextInputType.number),
                ]),
                ...
                _section('11', 'Studio photos (optional)', [
                  const Text('Photos help viewers choose you.', style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                  const SizedBox(height: 10),
                  _uploadTile(
                    '11.1 Gallery photo',
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : CreatorCatalog.formatTime(value)),
