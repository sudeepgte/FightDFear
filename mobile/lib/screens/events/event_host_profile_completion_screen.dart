...
class EventHostProfileCompletionScreen extends StatefulWidget {
  const EventHostProfileCompletionScreen({super.key, this.onFinished});
  ...
  State<EventHostProfileCompletionScreen> createState() =>
      _EventHostProfileCompletionScreenState();
}

class _EventHostProfileCompletionScreenState
    extends State<EventHostProfileCompletionScreen> {
  ...
  EventHostAuthService get _svc =>
      EventHostAuthService(context.read<AuthState>().api);
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _orgName.text = p['organizerName']?.toString() ?? '';
    ...
    _credential.text = p['credentialNumber']?.toString() ?? p['barCouncilId']?.toString() ?? '';
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', WomenEventCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(WomenEventCatalog.splitCsv(p['expertise'] ?? p['categoriesOffered']));
    _audience
      ..clear()
      ..addAll(WomenEventCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(WomenEventCatalog.splitCsv(p['facilities']));
    _days
      ..clear()
      ..addAll(WomenEventCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(WomenEventCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = WomenEventCatalog.parseTime(p['openTime']?.toString());
    _closeTime = WomenEventCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = WomenEventCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = WomenEventCatalog.parseTime(p['breakEnd']?.toString());
    ...
    if ((_sessionMode ?? '').isEmpty) _sessionMode = WomenEventCatalog.sessionModes.first;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'eventCategories': _categories.toList(),
      ...
      'openTime': _openTime == null ? null : WomenEventCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : WomenEventCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : WomenEventCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : WomenEventCatalog.formatTime(_breakEnd!),
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if (_orgName.text.trim().isEmpty) return '1.3 Organizer name is required';
    if ((_designation ?? '').isEmpty) return '1.2 Organizer type is required';
    ...
    if (_credential.text.trim().isEmpty) return '1.8 GST / NGO / CIN number is required';
    ...
    if (double.tryParse(_rate.text.trim()) == null) return '8. Typical ticket is required';
  ...
  Widget build(BuildContext context) {
    ...
    final missing = WomenEventCatalog.splitCsv(_profile['missingItems']);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: EventHostProfileCompletionScreen.navy,
        title: const Text('Complete Host Profile', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
      body: _loading
              ...
              children: [
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: EventHostProfileCompletionScreen.primary,
                ...
                _section('1', 'Host identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _field(_orgName, '1.3', 'Organizer name', required: true),
                  _dropdown(
                    number: '1.2',
                    label: 'Organizer type',
                    value: _designation,
                    options: WomenEventCatalog.designations,
                    required: true,
                  ...
                  _field(_credential, '1.8', 'GST / NGO / CIN number', required: true, hint: 'GST, NGO registration, or CIN'),
                ]),
                _section('2', 'Location', [
                  _field(_address, '2.1', 'Address', required: true),
                  _field(_city, '2.3', 'City', required: true),
                  _dropdown(number: '2.4', label: 'State', value: _state, options: WomenEventCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                  ...
                  const SizedBox(height: 8),
                  const Text('2.7 Pin office on map', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(WomenEventCatalog.audiences, _audience),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('4.2 Door / on-site events'),
                    value: _doorService,
                    onChanged: (v) => setState(() => _doorService = v),
                  ),
                ]),
                _section('5', 'Facilities', [
                  const Text('5.1 Venue amenities', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  _chips(WomenEventCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: WomenEventCatalog.days.map((d) {
                ...
                _section('8', 'Typical event offering', [
                  _dropdown(
                    number: '8.1',
                    label: 'Event mode',
                    value: _sessionMode,
                    options: WomenEventCatalog.sessionModes,
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_duration',
                    options: WomenEventCatalog.durations.map((e) => '$e').toList(),
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_buffer',
                    options: WomenEventCatalog.buffers.map((e) => '$e').toList(),
                    onChanged: (v) => setState(() => _buffer = int.tryParse(v ?? '10') ?? 10),
                  ),
                  _field(_rate, '8.4', 'Typical ticket (₹, 0 = free)', required: true, keyboardType: TextInputType.number),
                ]),
                ...
                _section('11', 'Event photos (optional)', [
                  const Text('Photos help attendees choose you.', style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                  const SizedBox(height: 10),
                  _uploadTile(
                    '11.1 Gallery photo',
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : WomenEventCatalog.formatTime(value)),
