...
class FinancialEducatorProfileCompletionScreen extends StatefulWidget {
  const FinancialEducatorProfileCompletionScreen({super.key, this.onFinished});
  ...
  @override
  State<FinancialEducatorProfileCompletionScreen> createState() =>
      _FinancialEducatorProfileCompletionScreenState();
}

class _FinancialEducatorProfileCompletionScreenState
    extends State<FinancialEducatorProfileCompletionScreen> {
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', FinancialCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(FinancialCatalog.splitCsv(p['expertise'] ?? p['categoriesOffered']));
    _audience
      ..clear()
      ..addAll(FinancialCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(FinancialCatalog.splitCsv(p['facilities']));
    _days
      ..clear()
      ..addAll(FinancialCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(FinancialCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = FinancialCatalog.parseTime(p['openTime']?.toString());
    _closeTime = FinancialCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = FinancialCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = FinancialCatalog.parseTime(p['breakEnd']?.toString());
    ...
    if ((_sessionMode ?? '').isEmpty) _sessionMode = FinancialCatalog.sessionModes.first;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'openTime': _openTime == null ? null : FinancialCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : FinancialCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : FinancialCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : FinancialCatalog.formatTime(_breakEnd!),
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if (_credential.text.trim().isEmpty) return '1.8 NISM / SEBI / IRDAI / CFP number is required';
    if (_categories.isEmpty) return '3.1 Select at least one expertise';
    ...
    if (double.tryParse(_rate.text.trim()) == null) return '8. Session fee is required';
  ...
  Widget build(BuildContext context) {
    ...
    final missing = FinancialCatalog.splitCsv(_profile['missingItems']);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: FinancialEducatorProfileCompletionScreen.navy,
        title: const Text('Complete Educator Profile', style: TextStyle(fontWeight: FontWeight.w700)),
      ),
      body: _loading
              ...
              children: [
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: FinancialEducatorProfileCompletionScreen.primary,
                ...
                _section('1', 'Educator identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _dropdown(
                    number: '1.2',
                    label: 'Designation',
                    value: _designation,
                    options: FinancialCatalog.designations,
                    required: true,
                  ...
                  _field(_credential, '1.8', 'NISM / SEBI / IRDAI / CFP number', required: true, hint: 'Enrolment / registration number'),
                ]),
                _section('2', 'Location', [
                  _field(_address, '2.1', 'Address', required: true),
                  _field(_city, '2.3', 'City', required: true),
                  _dropdown(number: '2.4', label: 'State', value: _state, options: FinancialCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                ...
                _section('3', 'Expertise', [
                  const Text('3.1 Expertise *', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: FinancialCatalog.expertise.map((c) {
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(FinancialCatalog.audiences, _audience),
                  SwitchListTile(
                ...
                _section('5', 'Facilities', [
                  ...
                  const SizedBox(height: 8),
                  _chips(FinancialCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: FinancialCatalog.days.map((d) {
                ...
                _section('8', 'First session offering', [
                  _dropdown(
                    number: '8.1',
                    label: 'Session mode',
                    value: _sessionMode,
                    options: FinancialCatalog.sessionModes,
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_duration',
                    options: FinancialCatalog.durations.map((e) => '$e').toList(),
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_buffer',
                    options: FinancialCatalog.buffers.map((e) => '$e').toList(),
                    onChanged: (v) => setState(() => _buffer = int.tryParse(v ?? '10') ?? 10),
                  ),
                  _field(_rate, '8.4', 'Typical fee (₹, 0 = free)', required: true, keyboardType: TextInputType.number),
                ]),
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : FinancialCatalog.formatTime(value)),
