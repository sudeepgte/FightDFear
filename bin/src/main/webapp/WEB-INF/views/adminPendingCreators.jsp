...
<head>
  ...
  <title>Pending Creators | Fight D Fear Admin</title>
...
<div class="page-wrap">
  <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
    <div>
      <h1 class="h4 mb-1">Pending Creator Hub applications</h1>
      <p class="text-muted mb-0 small">Review Join Us → Creator Hub submissions (${pendingCount})</p>
    </div>
  ...
  <div class="card-panel">
    <div class="table-responsive">
      <table class="table align-middle">
        ...
        <tbody>
        <c:choose>
          <c:when test="${not empty pendingCreators}">
            <c:forEach var="c" items="${pendingCreators}">
              <tr>
                <td>
                  <span class="fw-semibold d-block">${c.fullName}</span>
                  <small class="text-muted">${c.email}</small>
                  <div class="small text-muted mt-1">${c.creatorProfileCompletionPct != null ? c.creatorProfileCompletionPct : 0}% complete</div>
                  <c:if test="${not empty c.creatorBio}">
                    <div class="small text-muted mt-1">${c.creatorBio}</div>
                  </c:if>
                </td>
                <td>
                  <c:choose>
                    <c:when test="${c.creatorProfileStatus == 'PENDING_ADMIN_APPROVAL'}">
                      <span class="badge bg-warning text-dark">Pending approval</span>
                    </c:when>
                    <c:when test="${c.creatorProfileStatus == 'READY_FOR_VERIFICATION'}">
                      <span class="badge bg-info text-dark">Ready to submit</span>
                    </c:when>
                    <c:when test="${c.creatorProfileStatus == 'CHANGES_REQUESTED'}">
                      <span class="badge text-dark" style="background:#fdba74;">Changes requested</span>
                    </c:when>
                    <c:otherwise>
                      <span class="badge bg-secondary">${empty c.creatorProfileStatus ? 'Incomplete' : c.creatorProfileStatus}</span>
                    </c:otherwise>
                  </c:choose>
                </td>
                <td class="small">
                  <div>${empty c.creatorCategory ? '—' : c.creatorCategory}</div>
                  <div class="text-muted">${empty c.creatorCity ? 'No city' : c.creatorCity}</div>
                </td>
                <td class="small">${empty c.phoneNumber ? '—' : c.phoneNumber}</td>
                <td>
                  <div class="d-flex flex-wrap gap-2">
                    <form action="${pageContext.request.contextPath}/admin/creators/${c.id}/approve" method="post" class="m-0">
                      <button type="submit" class="btn-approve"><i class="fas fa-check me-1"></i>Approve</button>
                    </form>
                    <form action="${pageContext.request.contextPath}/admin/creators/${c.id}/reject" method="post" class="m-0"
                          onsubmit="return confirm('Reject this creator application?');">
                    ...
                    <form action="${pageContext.request.contextPath}/admin/creators/${c.id}/request-changes" method="post" class="m-0">
