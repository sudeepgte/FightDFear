...
class WomenJobsProfileCompletionScreen extends StatefulWidget {
  const WomenJobsProfileCompletionScreen({super.key, this.onFinished});
  ...
  @override
  State<WomenJobsProfileCompletionScreen> createState() =>
      _WomenJobsProfileCompletionScreenState();
}

class _WomenJobsProfileCompletionScreenState
    extends State<WomenJobsProfileCompletionScreen> {
  ...
  WomenJobsAuthService get _svc =>
      WomenJobsAuthService(context.read<AuthState>().api, context.read<AuthState>());

  @override
  void initState() {
    super.initState();
    _jobCategory = JobCatalog.categories.contains('Housekeeping')
        ? 'Housekeeping'
        : JobCatalog.categories.first;
    _load();
  }
  ...
  void dispose() {
    for (final c in [
      ...
      _pincode, _mapLocation, _bio, _upi, _bank, _rate,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  void _applyProfile(Map<String, dynamic> p) {
    ...
    _state = _pickOrOther(p['state']?.toString() ?? '', JobCatalog.indianStates, _stateOther);
    ...
    _categories
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['categoriesOffered']));
    if (_categories.isEmpty && p['jobCategory'] != null) {
      final cat = JobCatalog.normalize(p['jobCategory']?.toString());
      if (cat != null) _categories.add(cat);
    }
    _jobCategory = JobCatalog.normalize(p['jobCategory']?.toString()) ?? _jobCategory;
    _jobSubCategory = p['jobSubCategory']?.toString();
    if ((_jobSubCategory ?? '').isEmpty) _jobSubCategory = null;
    _audience
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['audience']));
    _facilities
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['facilities']));
    _languages
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['languages']));
    _skills
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['skills']));
    _days
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['openDays']));
    _blockedDates
      ..clear()
      ..addAll(JobCatalog.splitCsv(p['blockedDates']));
    _doorService = p['doorService'] == true;
    _openTime = JobCatalog.parseTime(p['openTime']?.toString());
    _closeTime = JobCatalog.parseTime(p['closeTime']?.toString());
    _breakStart = JobCatalog.parseTime(p['breakStart']?.toString());
    _breakEnd = JobCatalog.parseTime(p['breakEnd']?.toString());
    _bio.text = p['bio']?.toString() ?? '';
    _upi.text = p['upiId']?.toString() ?? '';
    _bank.text = p['bankDetails']?.toString() ?? '';
    if (p['hourlyRate'] != null) _rate.text = '${p['hourlyRate']}';
    ...
    _serviceMode = p['serviceMode']?.toString().isNotEmpty == true
        ...
        : 'DOOR';
    if (!JobCatalog.serviceModes.contains(_serviceMode)) _serviceMode = 'DOOR';
    _workType = p['workType']?.toString();
    if ((_workType ?? '').isEmpty) _workType = null;
  }
  ...
  Map<String, dynamic> _profileBody() {
    ...
    return {
      ...
      'jobCategory': _jobCategory ?? (_categories.isEmpty ? null : _categories.first),
      'jobSubCategory': _jobSubCategory,
      ...
      'openTime': _openTime == null ? null : JobCatalog.formatTime(_openTime!),
      'closeTime': _closeTime == null ? null : JobCatalog.formatTime(_closeTime!),
      'breakStart': _breakStart == null ? null : JobCatalog.formatTime(_breakStart!),
      'breakEnd': _breakEnd == null ? null : JobCatalog.formatTime(_breakEnd!),
  ...
  String? _validate({bool forSubmit = false}) {
    ...
    if (_categories.isEmpty) return '3.1 Select at least one work category';
    ...
    if ((_jobSubCategory ?? '').isEmpty) return '8. First offering role is required';
    if (double.tryParse(_rate.text.trim()) == null) return '8. Hourly rate is required';
          ...
          : ListView(
              ...
              children: [
                ...
                ProfileCompletionCard(
                  percent: pct,
                  statusLabel: _profile['applicationStatus']?.toString() ?? 'Pending',
                ...
                FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: WomenJobsProfileCompletionScreen.primary,
                ...
                _section('1', 'Worker identity', [
                  _field(_name, '1.1', 'Full name', required: true),
                  _dropdown(
                    number: '1.2',
                    label: 'Role type',
                    value: _designation,
                    options: JobCatalog.designations,
                    required: true,
                ...
                _section('2', 'Location', [
                  ...
                  _dropdown(number: '2.4', label: 'State', value: _state, options: JobCatalog.indianStates, required: true, onChanged: (v) => setState(() => _state = v)),
                ...
                _section('3', 'Work categories', [
                  const Text('3.1 Categories you offer *', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: JobCatalog.categories.map((c) {
                      ...
                      return FilterChip(
                        ...
                        onSelected: (v) => setState(() {
                          if (v) {
                            ...
                            if (_jobCategory == c) {
                              _jobCategory = _categories.isEmpty ? null : _categories.first;
                              _jobSubCategory = null;
                            }
                          }
                        }),
                      );
                ...
                _section('4', 'Who I serve', [
                  ...
                  const SizedBox(height: 8),
                  _chips(JobCatalog.audiences, _audience),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('4.2 Door / home visits'),
                    value: _doorService,
                  ...
                  const SizedBox(height: 8),
                  _chips(JobCatalog.languages, _languages),
                  const SizedBox(height: 8),
                  const Text('4.4 Skills', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  _chips(JobCatalog.skills, _skills),
                ]),
                _section('5', 'Facilities & readiness', [
                  const Text('5.1 Amenities / readiness', style: TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                  const SizedBox(height: 8),
                  _chips(JobCatalog.facilities, _facilities),
                ]),
                _section('6', 'Hours & calendar', [
                  ...
                  Wrap(
                    spacing: 8,
                    children: JobCatalog.days.map((d) {
                ...
                _section('8', 'First offering', [
                  ...
                  _dropdown(
                    number: '8.2',
                    label: 'Role / service',
                    value: _jobSubCategory,
                    options: subs,
                    required: true,
                    onChanged: (v) => setState(() => _jobSubCategory = v),
                  ),
                  _dropdown(
                    number: '8.3',
                    label: 'Duration (minutes)',
                    value: '$_duration',
                    options: JobCatalog.durations.map((e) => '$e').toList(),
                    required: true,
                  ...
                  _dropdown(
                    ...
                    value: '$_buffer',
                    options: JobCatalog.buffers.map((e) => '$e').toList(),
                    onChanged: (v) => setState(() => _buffer = int.tryParse(v ?? '10') ?? 10),
                  ),
                  _field(_rate, '8.5', 'Hourly rate (₹)', required: true, keyboardType: TextInputType.number),
                  _dropdown(
                    number: '8.6',
                    label: 'Mode',
                    value: _serviceMode,
                    options: JobCatalog.serviceModes,
                    required: true,
                    onChanged: (v) => setState(() => _serviceMode = v ?? 'DOOR'),
                  ),
                  _dropdown(
                    number: '8.7',
                    label: 'Work type',
                    value: _workType,
                    options: JobCatalog.workTypes,
  ...
  Widget _timeTile(String label, TimeOfDay? value, void Function(TimeOfDay t) apply) {
    return Padding(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        title: Text(label),
        subtitle: Text(value == null ? 'Tap to pick' : JobCatalog.formatTime(value)),
