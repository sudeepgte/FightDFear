...
class WomenLawyerDetailScreen extends StatefulWidget {
  const WomenLawyerDetailScreen({super.key, required this.lawyerId});
  ...
  @override
  State<WomenLawyerDetailScreen> createState() =>
      _WomenLawyerDetailScreenState();
}

class _WomenLawyerDetailScreenState extends State<WomenLawyerDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.providerDetail(widget.lawyerId);
      if (!mounted) return;
      if (res['success'] == true && res['provider'] is Map) {
        _lawyer = Map<String, dynamic>.from(res['provider'] as Map);
        _reviews = _toList(res['reviews']);
        _slotsToday = (res['slotsToday'] is List)
            ? (res['slotsToday'] as List).map((e) => e.toString()).toList()
            : <String>[];
        final gallery = _lawyer?['galleryPhotos'];
        _photos = LawyerCatalog.splitCsv(gallery);
        _favorite = res['favorite'] == true || _lawyer?['favorite'] == true;
        ...
        _cancelPolicy = res['cancelPolicy']?.toString() ?? LawyerCatalog.cancelPolicy;
      } else {
        _error = res['error']?.toString() ?? 'Could not load lawyer';
      }
    } catch (e) {
      _error = '$e';
    }
  ...
  Future<void> _writeReview() async {
    ...
    if (ok != true) return;
    final res = await _api.addProviderReview(widget.lawyerId, rating: rating, comment: comment.text.trim());
    if (!mounted) return;
  ...
  Future<void> _book() async {
    final w = _lawyer;
    if (w == null) return;
    final rate = (w['consultationFee'] is num) ? (w['consultationFee'] as num).toDouble() : 0.0;
    ...
    final payload = await showDialog<Map<String, String>>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => AlertDialog(
          title: Text('Book ${w['fullName'] ?? 'Lawyer'}'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text(_cancelPolicy ?? LawyerCatalog.cancelPolicy,
                    style: const TextStyle(fontSize: 12, color: WomenLawyerDetailScreen.textGray)),
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    final slotRes = await _api.providerSlots(widget.lawyerId, date: key);
                    setLocal(() {
                      pickedDate = d;
                ...
                if (slots.isEmpty)
                  const Text('No slots on this date',
                      style: TextStyle(color: WomenLawyerDetailScreen.textGray))
                else
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        foregroundColor: WomenLawyerDetailScreen.navy,
        title: Text(w?['fullName']?.toString() ?? 'Lawyer'),
        actions: [
          IconButton(
            icon: Icon(_favorite ? Icons.favorite : Icons.favorite_border,
                color: WomenLawyerDetailScreen.primary),
            onPressed: () async {
              final res = await _api.toggleLawyerFavorite(widget.lawyerId);
              if (!mounted) return;
              if (res['success'] == true) {
                    ...
                    children: [
                      ClipRRect(
                        ...
                        child: image.isEmpty
                            ? Container(
                                ...
                                child: const Icon(Icons.gavel_outlined, size: 44),
                              )
                            : Image.network(image, height: 180, fit: BoxFit.cover),
                      ),
                      const SizedBox(height: 12),
                      Text(w?['fullName']?.toString() ?? '',
                          style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
                      Text('${w?['city'] ?? ''} ${w?['locationText'] ?? ''}',
                          style: const TextStyle(color: WomenLawyerDetailScreen.textGray)),
                      const SizedBox(height: 8),
                      Wrap(
                        spacing: 8,
                        runSpacing: 6,
                        children: [
                          ...LawyerCatalog.splitAreas(w?['practiceAreas']?.toString())
                              .take(3)
                              .map((a) => Chip(label: Text(a))),
                          if (w?['consultationFee'] != null) Chip(label: Text('₹${w?['consultationFee']}')),
                          if (w?['consultationMode'] != null) Chip(label: Text('${w?['consultationMode']}')),
                          if (w?['rating'] != null) Chip(label: Text('★ ${w?['rating']}')),
                          if (_nextSlotLabel != null) Chip(label: Text(_nextSlotLabel!)),
                          if (w?['doorService'] == true) const Chip(label: Text('Home / chamber visit')),
                      ...
                      FilledButton(
                        ...
                        style: FilledButton.styleFrom(
                          backgroundColor: WomenLawyerDetailScreen.primary,
                          minimumSize: const Size.fromHeight(48),
                        ),
                        child: const Text('Request consult'),
                      ),
                      const SizedBox(height: 8),
